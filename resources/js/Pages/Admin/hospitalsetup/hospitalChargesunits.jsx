...
const hospitalChargesunits = ({ admin }) => {
    ...
    const fetchData = () => {
        axios.post('/api/admin/hospitalChargeUnit-fetch').then(res => {
            console.log(res)
            setData(res.data)
    ...
    const handleSubmit = async (e) => {
        ...
        formSave.append('unit_name', formData.unit_name)
        ...
        try {
            const response = await axios.post('/api/admin/hospitalChargeUnit-store', formSave);
    ...
    const Updatedata = (e, id) => {
        ...
        axios.post(`/api/admin/hospitalChargeUnit-update/${id}`, formSave).then(res => fetchData()).catch(error => console.log(error))
    }
    const DeleteData = (e, id) => {
        e.preventDefault();

        axios.post(`/api/admin/hospitalChargeUnit-delete/${id}`)
            .then(response => {
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {
                                            data.map((datas, idx) => (
                                                <tr>
                                                    <td className='px-6 py-3 text-left text-xs'>{datas.unit_name}</td>
                                                                ...
                                                                <div className="modal-body">
                                                                    ...
                                                                    <form action className="w-full grid grid-cols-1 gap-5 px-6 mt-10 relative">
                                                                        <div className="form-group w-full ">
                                                                            ...
                                                                            <input onChange={handleChange} name='unit_name' value={formData.unit_name} type="text" className="w-full border-gray-300" />
                                                                        </div>
...
export default hospitalChargesunits;