...
const itemStore = ({ admin }) => {
    ...
    const fetchData = () => {
        axios.post('/api/admin/inventoryStore-fetch').then(res => {
            console.log(res)
            setData(res.data)
    ...
    const handleSubmit = async (e) => {
        ...
        formSave.append('instore_name', formData.instore_name)
        ...
        try {
            const response = await axios.post('/api/admin/inventoryStore-store', formSave);
    ...
    const Updatedata = (e, id) => {
        ...
        axios.post(`/api/admin/inventoryStore-update/${id}`, formSave).then(res => fetchData()).catch(error => console.log(error))
    }
    const DeleteData = (e, id) => {
        e.preventDefault();

        axios.post(`/api/admin/inventoryStore-delete/${id}`)
            .then(response => {
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {
                                            data.map((datas, idx) => (
                                                <tr>
                                                    <td className='px-6 py-3 text-left text-xs'>{datas.instore_name}</td>
                                                    ...
                                                    <td td className='px-6 py-3  text-xs flex space-x-2 text-end justify-end' >
                                                        <a onClick={() => {
                                                            setFormData({
                                                                ...
                                                                instore_name: datas.instore_name,
                                                                ...
                                                                <div className="modal-body">
                                                                    ...
                                                                    <form action className="w-full grid grid-cols-1 gap-5 px-6 mt-10 relative">
                                                                        <div className="form-group w-full ">
                                                                            ...
                                                                            <input onChange={handleChange} name='instore_name' value={formData.instore_name} type="text" className="w-full border-gray-300" />
                                                                        </div>
