...
const bloodBnkproduct = ({ admin, units }) => {
    ...
    const fetchData = () => {
        axios.post('/api/admin/bloodbankProduct-fetch').then(res => {
            console.log(res)
            setData(res.data)
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('/api/admin/bloodbankProduct-store', formSave);
    ...
    const Updatedata = (e, id) => {
        ...
        axios.post(`/api/admin/bloodbankProduct-update/${id}`, formSave).then(res => fetchData()).catch(error => console.log(error))
    }
    const DeleteData = (e, id) => {
        e.preventDefault();

        axios.post(`/api/admin/bloodbankProduct-delete/${id}`)
            .then(response => {
                                                                ...
                                                                <div className="modal-body">
                                                                    ...
                                                                    <form action className="w-full grid grid-cols-1 gap-5 px-6 mt-10 relative">
                                                                        ...
                                                                        <div className="form-group w-full">
                                                                            ...
                                                                            <select onChange={handleChange} name='type_id' value={formData.type_id} className="w-full border-gray-300" >
                                                                                <option value=""> Select</option>
                                                                                {
                                                                                    units.map(i => (
                                                                                        <option value={i.id}>{i.blood_type}</option>
                                                                                    ))
                                                                                }
                                                                            </select>
                                                                        </div>
...
export default bloodBnkproduct;