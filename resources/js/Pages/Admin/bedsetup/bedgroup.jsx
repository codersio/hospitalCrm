...
const BedGroup = ({ admin, floor }) => {
    ...
    const fetchData = () => {
        axios.post('/api/admin/bed-fetch').then(res => {
            console.log(res)
            setData(res.data)
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('/api/admin/bed-store', formSave);
    ...
    const Updatedata = (e, id) => {
        ...
        axios.post(`/api/admin/bed-update/${id}`, formSave).then(res => fetchData()).catch(error => console.log(error))
    }
    const DeleteData = (e, id) => {
        e.preventDefault();

        axios.post(`/api/admin/bed-delete/${id}`)
            .then(response => {
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {
                                            data.map((datas, idx) => (
                                                <tr>
                                                    ...
                                                    <td className='px-6 py-3 text-left text-xs'>{datas.floor_id}</td>
                                                                ...
                                                                <div className="modal-body">
                                                                    ...
                                                                    <form action className="w-full grid grid-cols-1 gap-5 px-6 mt-10 relative">
                                                                        ...
                                                                        <div className="form-group w-full">
                                                                            ...
                                                                            <input onChange={handleChange} name='group_name' value={formData.group_name} type="text" className="w-full border-gray-300" />
                                                                        </div>
                                                                        <div className="form-group w-full">
                                                                            <label htmlFor> Floor *</label> <br />
                                                                            <select onChange={handleChange} name='floor_id' value={formData.floor_id} type="text" className="w-full border-gray-300">
                                                                                <option value="">Select</option>
                                                                                {
                                                                                    floor.map((datas, idx) => (
                                                                                        <option value={datas.id}>{datas.floor_name}</option>
                                                                                    ))
                                                                                }
                                                                            </select>
