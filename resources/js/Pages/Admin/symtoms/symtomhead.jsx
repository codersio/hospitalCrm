...
const Symtomshead = ({ admin, symtoms }) => {
    ...
    const fetchData = () => {
        axios.post('/api/admin/symtomsHead-fetch').then(res => {
            console.log(res)
            setData(res.data)
    ...
    const handleSubmit = async (e) => {
        ...
        formSave.append('symptom_head', formData.symptom_head)
        formSave.append('type_id', formData.type_id)
        ...
        try {
            const response = await axios.post('/api/admin/symtomsHead-store', formSave);
    ...
    const Updatedata = (e, id) => {
        ...
        axios.post(`/api/admin/symtomsHead-update/${id}`, formSave).then(res => fetchData()).catch(error => console.log(error))
    }
    const DeleteData = (e, id) => {
        e.preventDefault();

        axios.post(`/api/admin/symtomsHead-delete/${id}`)
            .then(response => {
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {
                                            data.map((datas, idx) => (
                                                <tr>
                                                    ...
                                                    <td className='px-6 py-3 text-left text-xs'>{datas.symptom_head}</td>
                                                    ...
                                                    <td td className='px-6 py-3  text-xs flex space-x-2 text-end justify-end' >
                                                        <a onClick={() => {
                                                            setFormData({
                                                                symptom_head: datas.symptom_head,
                                                                ...
                                                                <div className="modal-body">
                                                                    ...
                                                                    <form action className="w-full grid grid-cols-1 gap-5 px-6 mt-10 relative">
                                                                        ...
                                                                        <div className="form-group w-full">
                                                                            <label htmlFor> Symptoms Head *</label> <br />
                                                                            <input onChange={handleChange} name='symptom_head' value={formData.symptom_head} type="text" className="w-full border-gray-300" />
                                                                        </div>
                                                                        <div className="form-group w-full">
                                                                            <label htmlFor>Symptoms Type *</label> <br />
                                                                            <select onChange={handleChange} name="type_id" value={formData.type_id} className="w-full border-gray-300" id="">
                                                                                <option value="">Select</option>
                                                                                {symtoms.map(sy => (
                                                                                    <option value={sy.id}>{sy.symptom_name}</option>
                                                                                ))}
                                                                            </select>
                                                                        </div>
                                                                        <div className="form-group w-full">
