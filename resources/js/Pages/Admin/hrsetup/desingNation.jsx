...
const DesingNation = ({ admin }) => {
    ...
    const fetchData = () => {
        axios.post('/api/admin/humanResourceDesingation-fetch').then(res => {
            console.log(res)
            setData(res.data)
    ...
    const handleSubmit = async (e) => {
        ...
        formSave.append('degnation_name', formData.degnation_name)
        ...
        try {
            const response = await axios.post('/api/admin/humanResourceDesingation-store', formSave);
    ...
    const Updatedata = (e, id) => {
        ...
        axios.post(`/api/admin/humanResourceDesingation-update/${id}`, formSave).then(res => fetchData()).catch(error => console.log(error))
    }
    const DeleteData = (e, id) => {
        e.preventDefault();

        axios.post(`/api/admin/humanResourceDesingation-delete/${id}`)
            .then(response => {
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {
                                            data.map((datas, idx) => (
                                                <tr>
                                                    <td className='px-6 py-3 text-left text-xs'>{datas.degnation_name}</td>
                                                    ...
                                                    <td td className='px-6 py-3  text-xs flex space-x-2 text-end justify-end' >
                                                        <a onClick={() => {
                                                            setFormData({
                                                                ...
                                                                degnation_name: datas.degnation_name,
                                                                ...
                                                                <div className="modal-body">
                                                                    ...
                                                                    <form action className="w-full grid grid-cols-1 gap-5 px-6 mt-10 relative">
                                                                        <div className="form-group w-full ">
                                                                            ...
                                                                            <input onChange={handleChange} name='degnation_name' value={formData.degnation_name} type="text" className="w-full border-gray-300" />
                                                                        </div>
