...
const radiologyParameter = ({ admin, units }) => {
    ...
    const fetchData = () => {
        axios.post('/api/admin/radiologyParameters-fetch').then(res => {
            console.log(res)
            setData(res.data)
    ...
    const handleSubmit = async (e) => {
        ...
        formSave.append('units_id', formData.units_id)
        formSave.append('parameter_name', formData.parameter_name)
        formSave.append('range', formData.range)
        ...
        try {
            const response = await axios.post('/api/admin/radiologyParameters-store', formSave);
    ...
    const Updatedata = (e, id) => {
        ...
        axios.post(`/api/admin/radiologyParameters-update/${id}`, formSave).then(res => fetchData()).catch(error => console.log(error))
    }
    const DeleteData = (e, id) => {
        e.preventDefault();

        axios.post(`/api/admin/radiologyParameters-delete/${id}`)
            .then(response => {
                            ...
                            <div className="card mt-2">
                                <div className="card-header flex justify-between px-[3rem] border py-3">
                                    <div className="grid place-items-center text-[22px]">
                                        <h1>Pathology Parameter List </h1>
                                    </div>
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {
                                            data.map((datas, idx) => (
                                                <tr>
                                                    <td className='px-6 py-3 text-left text-xs'>{datas.parameter_name}</td>
                                                    <td className='px-6 py-3 text-left text-xs'>{datas.units_id}</td>
                                                    <td className='px-6 py-3 text-left text-xs'>{datas.range}</td>
                                                    ...
                                                    <td td className='px-6 py-3  text-xs flex space-x-2 text-end justify-end' >
                                                        <a onClick={() => {
                                                            setFormData({
                                                                ...
                                                                parameter_name: datas.parameter_name,
                                                                range: datas.range,
                                                                ...
                                                                <div className="modal-body">
                                                                    ...
                                                                    <form action className="w-full grid grid-cols-1 gap-5 px-6 mt-10 relative">
                                                                        ...
                                                                        <div className="form-group w-full">
                                                                            <label htmlFor>Parameter Name *</label> <br />
                                                                            <input onChange={handleChange} name='parameter_name' value={formData.parameter_name} type="text" className="w-full border-gray-300" />
                                                                        </div>
                                                                        <div className="form-group w-full">
                                                                            <label htmlFor>Reference Range *</label> <br />
                                                                            <input onChange={handleChange} name='range' value={formData.range} type="text" className="w-full border-gray-300" />
                                                                        </div>
                                                                        <div className="form-group w-full">
                                                                            <label htmlFor>Unit *</label> <br />
                                                                            <select onChange={handleChange} name='units_id' value={formData.units_id} className="w-full border-gray-300" >
                                                                                <option value=""> Select</option>
                                                                                {
                                                                                    units.map(i => (
                                                                                        <option value={i.id}>{i.radio_unit_name}</option>
                                                                                    ))
                                                                                }
                                                                            </select>
                                                                        </div>
...
export default radiologyParameter;