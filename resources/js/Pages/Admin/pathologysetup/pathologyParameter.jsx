...
const pathologyParameter = ({ admin, units }) => {
    ...
    const fetchData = () => {
        axios.post('/api/admin/pathologyparameter-fetch').then(res => {
            console.log(res)
            setData(res.data)
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('/api/admin/pathologyparameter-store', formSave);
    ...
    const Updatedata = (e, id) => {
        ...
        axios.post(`/api/admin/pathologyparameter-update/${id}`, formSave).then(res => fetchData()).catch(error => console.log(error))
    }
    const DeleteData = (e, id) => {
        e.preventDefault();

        axios.post(`/api/admin/pathologyparameter-delete/${id}`)
            .then(response => {
                                                                    ...
                                                                    <form action className="w-full grid grid-cols-1 gap-5 px-6 mt-10 relative">
                                                                        ...
                                                                        <div className="form-group w-full">
                                                                            ...
                                                                            <select onChange={handleChange} name='units_id' value={formData.units_id} className="w-full border-gray-300" >
                                                                                ...
                                                                                {
                                                                                    units.map(i => (
                                                                                        <option value={i.id}>{i.patho_unit_name}</option>
                                                                                    ))
                                                                                }
                                                                            </select>
                                                                        </div>
