...
const MedicineSupplier = ({ admin }) => {
    ...
    const fetchData = () => {
        axios.post('/api/admin/medicinesupplier-fetch').then(res => {
            console.log(res)
            setData(res.data)
    ...
    const handleSubmit = async (e) => {
        ...
        formSave.append('supp_name', formData.supp_name)
        formSave.append('supp_contact', formData.supp_contact)
        formSave.append('contact_person_name', formData.contact_person_name)
        formSave.append('contact_person_phone', formData.contact_person_phone)
        formSave.append('drug_licence', formData.drug_licence)
        formSave.append('address', formData.address)

        try {
            const response = await axios.post('/api/admin/medicinesupplier-store', formSave);
    ...
    const Updatedata = (e, id) => {
        ...
        axios.post(`/api/admin/medicinesupplier-update/${id}`, formSave).then(res => fetchData()).catch(error => console.log(error))
    }
    const DeleteData = (e, id) => {
        e.preventDefault();

        axios.post(`/api/admin/medicinesupplier-delete/${id}`)
            .then(response => {
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {
                                            data.map((datas, idx) => (
                                                <tr>
                                                    <td className='px-6 py-3 text-left text-xs'>{datas.supp_name}</td>
                                                    <td className='px-6 py-3 text-left text-xs'>{datas.supp_contact}</td>
                                                    <td className='px-6 py-3 text-left text-xs'>{datas.contact_person_name}</td>
                                                    <td className='px-6 py-3 text-left text-xs'>{datas.contact_person_phone}</td>
                                                    <td className='px-6 py-3 text-left text-xs'>{datas.drug_licence}</td>
                                                    ...
                                                    <td td className='px-6 py-3  text-xs flex space-x-2 text-end justify-end' >
                                                        <a onClick={() => {
                                                            setFormData({
                                                                supp_name: datas.supp_name,
                                                                supp_contact: datas.supp_contact,
                                                                contact_person_name: datas.contact_person_name,
                                                                contact_person_phone: datas.contact_person_phone,
                                                                drug_licence: datas.drug_licence,
                                                                    ...
                                                                    <form action className="w-full grid grid-cols-1 gap-5 px-6 mt-10 relative">
                                                                        ...
                                                                        <div className="grid grid-cols-2 gap-3">
                                                                            ...
                                                                            <div className="form-group w-full">
                                                                                ...
                                                                                <input onChange={handleChange} name='supp_name' value={formData.supp_name} type="text" className="w-full border-gray-300" />
                                                                            </div>
                                                                            <div className="form-group w-full">
                                                                                <label htmlFor>Supplier Contact</label> <br />
                                                                                <input onChange={handleChange} name='supp_contact' value={formData.supp_contact} type="text" className="w-full border-gray-300" />
                                                                            </div>
                                                                            <div className="form-group w-full">
                                                                                <label htmlFor>Contact Person Name</label> <br />
                                                                                <input onChange={handleChange} name='contact_person_name' value={formData.contact_person_name} type="text" className="w-full border-gray-300" />
                                                                            </div>
                                                                            <div className="form-group w-full">
                                                                                <label htmlFor>Contact Person Phone</label> <br />
                                                                                <input onChange={handleChange} name='contact_person_phone' value={formData.contact_person_phone} type="text" className="w-full border-gray-300" />
                                                                            </div>
                                                                            <div className="form-group w-full">
                                                                                <label htmlFor>Drug License Number</label> <br />
                                                                                <input onChange={handleChange} name='drug_licence' value={formData.drug_licence} type="text" className="w-full border-gray-300" />
                                                                            </div>
                                                                            <div className="form-group w-full">
                                                                                <label htmlFor>Address</label> <br />
                                                                                <input onChange={handleChange} name='address' value={formData.address} type="text" className="w-full border-gray-300" />
                                                                            </div>
                                                                        </div>
