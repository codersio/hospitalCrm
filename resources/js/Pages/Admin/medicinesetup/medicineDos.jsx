...
const MedicineDos = ({ admin, category, units }) => {
    ...
    const fetchData = () => {
        axios.post('/api/admin/medicineDose-fetch').then(res => {
            console.log(res)
            setData(res.data)
    ...
    const handleSubmit = async (e) => {
        ...
        formSave.append('hospital_units_id', formData.hospital_units_id)
        formSave.append('dose', formData.dose)



        try {
            const response = await axios.post('/api/admin/medicineDose-store', formSave);
    ...
    const Updatedata = (e, id) => {
        ...
        axios.post(`/api/admin/medicineDose-update/${id}`, formSave).then(res => fetchData()).catch(error => console.log(error))
    }
    const DeleteData = (e, id) => {
        e.preventDefault();

        axios.post(`/api/admin/medicineDose-delete/${id}`)
            .then(response => {
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {
                                            data.map((datas, idx) => (
                                                <tr>
                                                    ...
                                                    <td className='px-6 py-3 text-left text-xs'>{datas.dose}</td>
                                                    ...
                                                    <td td className='px-6 py-3  text-xs flex space-x-2 text-end justify-end' >
                                                        <a onClick={() => {
                                                            setFormData({
                                                                ...
                                                                hospital_units_id: datas.hospital_units_id,
                                                                dose: datas.dose,
                                                                    ...
                                                                    <form action className="w-full grid grid-cols-1 gap-5 px-6 mt-10 relative">
                                                                        ...
                                                                        <div className="grid grid-cols-2 gap-3">
                                                                            ...
                                                                            <div className="form-group w-full">
                                                                                <label htmlFor>Medicine Category *</label> <br />
                                                                                <select onChange={handleChange} name='category_id' value={formData.category_id} className="w-full border-gray-300">
                                                                            ...
                                                                            <div className="form-group w-full">
                                                                                <label htmlFor>Dose *</label> <br />
                                                                                <input onChange={handleChange} name='dose' value={formData.dose} type="text" className="w-full border-gray-300" />
                                                                            </div>
                                                                            <div className="form-group w-full">
                                                                                <label htmlFor>Unit *</label> <br />
                                                                                <select onChange={handleChange} name='hospital_units_id' value={formData.hospital_units_id} className="w-full border-gray-300">
                                                                                    <option value="">Select Category</option>
                                                                                    {
                                                                                        units.map(cat => (
                                                                                            <option key={cat.id} value={cat.id}>{cat.unit_name}</option>
                                                                                        ))
                                                                                    }
                                                                                </select>
                                                                            </div>
