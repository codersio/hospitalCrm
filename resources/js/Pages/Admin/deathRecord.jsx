...
const DeathRecord = ({ admin }) => {
    ...
    const fetchData = () => {
        axios.post('/api/admin/deathcertificate-fetch')
            .then(res => {
    ...
    const FormSubmit = (e) => {
        ...
        formSave.append('case_id', formData.case_id)
        ...
        formSave.append('guardian_name', formData.guardian_name)
        ...
        axios.post('/api/admin/deathcertificate-store', formSave,)
            .then(response => {
    ...
    const DeleteData = (e, id) => {
        ...
        axios.post(`/api/admin/deathcertificate-delete/${id}`)
            .then(response => {
                console.log(response)
                fetchData();
            })
    ...
    const Updatedata = (e, id) => {
        ...
        formSave.append('report', formData.report)
        formSave.append('atach_file', atach_file)
        axios.post(`/api/admin/deathcertificate-update/${id}`, formSave).then(res => fetchData()).catch(error => console.log(error))
    }
    return (
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {
                                    data.map((datas, idx) => (
                                        <tr>
                                            ...
                                            <td className='px-6 py-3 text-left text-xs'>{datas.case_id}</td>
                                            <td className='px-6 py-3 text-left text-xs'>{datas.patient_id}</td>
                                            <td className='px-6 py-3 text-left text-xs'>{datas.date}</td>
                                            <td className='px-6 py-3 text-left text-xs'>{datas.guardian_name}</td>
                                            ...
                                            <td className='px-6 py-3 text-left text-xs flex space-x-2'>
                                                <a onClick={() => {
                                                    setFormdata({
                                                        ...
                                                        date: datas.date,
                                                        guardian_name: datas.guardian_name,
                                                        report: datas.report,
                                                        ...
                                                        <div className="modal-body">
                                                            ...
                                                            <form action className="w-full grid grid-cols-2 gap-5 px-6 mt-10 relative">
                                                                <div className="form-group w-full ">
                                                                    ...
                                                                    <select onChange={handleFormdata} name='patient_id' value={formData.patient_id} type="text" className="w-full border-gray-300" >
                                                                        <option value="">select Patient </option>
                                                                        {

                                                                            Patientsdata.map((datas, index) => (
                                                                                <option value={datas.id} key={index}>{datas.name}</option>
                                                                            ))
                                                                        }
                                                                    </select>
                                                                </div>
                                                                <div className="form-group w-full">
                                                                    ...
                                                                    <input onChange={handleFormdata} name='case_id' value={formData.case_id} type="text" className="w-full border-gray-300" />
                                                                </div>
                                                                <div className="form-group w-full">
                                                                    <label htmlFor> Death Date *</label> <br />
                                                                    <input type="date" name='date' onChange={handleFormdata} value={formData.date} className="w-full border-gray-300" />
                                                                </div>
                                                                <div className="form-group w-full">
                                                                    <label htmlFor>Guardian Name *</label> <br />
                                                                    <input onChange={handleFormdata} name='guardian_name' value={formData.guardian_name} type="text" className="w-full border-gray-300" />
                                                                </div>
                                                                <div className="form-group w-full">
                                                                ...
                                                                <div className="form-group w-full">
                                                                    ...
                                                                    <textarea type="text" name='report' onChange={handleFormdata} value={formData.report} className="w-full border-gray-300" ></textarea>
